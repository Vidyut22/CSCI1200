...
private:
  ...
  void resize_table(unsigned int new_size) {
      ...
      vector<list<KeyType>> newTable(new_size);
      for (int i = 0; i < m_table.size(); i++)
      {
          for (hash_list_itr itr = m_table[i].begin(); itr != m_table[i].end();itr++) 
          {
              unsigned int hash_value = m_hash(*itr);
              unsigned int index = hash_value % new_size;

              newTable[index].push_front(*itr);
          }
      }
      m_table = newTable;
  }
