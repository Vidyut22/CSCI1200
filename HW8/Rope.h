...
private:
	...
	void destroy_rope(Node* p); //Helper method to delete entire rope given root
	Node* copy_rope(Node* old_root); //Helper method to copy entire rope gicen root
	void nodesWeight(Node* p, int& count); //Helper method to attain weight of any given node in rope
	Node* nodeofIndex(int& incount) const;// Helper method that returns the node of given index
	void fillInParents(Node*& p); //Helper method to fill in the parent pointers of all nodes in rope
	void concatrightsubtree(Node* curr, int ogweight); //Helper method to split and concat 
	void concat_not_copy(Node*& p); //Concat without copying
	void getSize(Node* p, int& size); //Helper method that returns the size of subtree
