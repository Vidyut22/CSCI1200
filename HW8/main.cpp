...
int main(){
	...
	cout << "STARTING STUDENT TESTS" << endl;
	StudentTests();
...
void StudentTests()
{
	//TESTING IN THE MIDDLE OF A NODE SPLITTING
	Rope r1(IndexTestRope());
	std::string s1 = "Hello my name is Simon";
	std::string s5 = s1.substr(0, 12);
	std::string s6 = s1.substr(12, 10);
	Rope r5;
	r1.split(12, r5);
	...
	//TESTING REPORT SAME CHARACTER
	string rs1 ="";
	string s3 = to_string(s1[0]);
	r1.report(0, 0, rs1);
	std::cout << "***Same character test passed" << std::endl;
}
