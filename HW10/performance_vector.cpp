...
void vector_longest_substring(std::vector<std::string> &vec, std::ostream &ostr, int &output_count) 
{
    int bigest_size = 0;
    string long_substr = "";
    //Loopp through all elements in vector
    for(int i = 0; i<vec.size()-1;i++)
    {
        for (int j = 0; j < vec[i].size(); j++)
        {
            //Get each substring in each word
            std::string sub = vec[i].substr(0, vec[i].size() - j);
            if (sub.size() > bigest_size)
            {
                //Compare each substring to each other substring in every other word
                for (int k = i + 1; k < vec.size(); k++)
                {
                    bool reached = false;
                    for (int l = 0; l < (int)vec.size() - sub.size(); l++)
                    {
                        string subsub = vec[k].substr(0, sub.size());
                        if (subsub == sub)
                        {
                            bigest_size = sub.size();
                            long_substr = sub;
                            reached = true;
                            break;
                        }
                    }
                    //Once reached end break
                    if (reached)
                    {
                        break;
                    }
