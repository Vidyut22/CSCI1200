...
void Team::operator+=(const Superhero& a) //add to
{
    superheroes.push_back(a);
}
void Team::operator-=(const Superhero& a) //remove
{
    superheroes.remove(a);
}

Team Team::operator+(const Team& a, const Team& b) //merge
{
    for (int i = 0; i < (int)b.superheroes.size();i++)
    {
        a.superheroes.push_back(b.superheroes[i]);
    }
    return a;
}
friend Team Team::operator+(const Team& a, const Superhero& b) //add to
{
    a.superheroes.push_back(b);
}

Team operator+(const Superhero& a, const Superhero& b) //build team
{
    Team c = Team();
    c.superheroes.push_back(a);
