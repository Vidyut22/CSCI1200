...
int pathfinder(std::vector<std::vector<bool> >& blocked_grid, int x, int y, vector<Point>& pts) {
    int k, j;
    if (blocked_grid[x][y]) 
    { 
        return 0; 
    ...
    { 
        k = pathfinder(blocked_grid, x, y - 1, pts); 
        if (pts.size() > 0 && delete_path)
        {
            pts.pop_back();
        }
        else if (k)
    ...
    { 
        k = pathfinder(blocked_grid, x - 1, y, pts); 
        if (pts.size() > 0 && delete_path)
        {
            pts.pop_back();
        }
    ...
    k = pathfinder(blocked_grid, x - 1, y, pts);
    j = pathfinder(blocked_grid, x, y - 1, pts);
    ...
    if (pts.size() > 0 && delete_path)
    {
        ...
        delete_path = true;
        pathfinder(blocked_grid, x, y - 1, pts);
        pts.push_back(Point(x, y));
        delete_path = false;
    }
    else if (k || j)
...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y) {
    ...
    for (int y = 0; y < (int)blocked_grid[0].size(); ++y) {
        for (int x = 0; x < (int)blocked_grid.size(); ++x) {
...
void print_vector(vector<vector<char> > const& v) {
...
void print_out_grid(vector<vector<bool> > const& blocked_grid, vector<Point> const& p, int start_x, int start_y) {

    vector<vector<char> > p_out;
    vector<char> temp;
    cout << "Here is the grid with the origin in the upper left corner, x increasing \n"
        << "horizontally and y increasing down the screen.  An 'X' represents a blocked\n"
        << "location and the 'S' represents the starting location.\n\n";
    std::cout << "The size of the list is " << p.size() << endl;
    for (int y = 0; y < (int)blocked_grid[0].size(); ++y) {
...
int main(int argc, char* argv[]) {
    ...
    int k = pathfinder(blocked_grid, start_x, start_y, pts);
    std::cout << k << std::endl;

    print_out_grid(blocked_grid, pts, start_x, start_y);

    for (unsigned i = 0; i < pts.size(); ++i) {
        cout << pts[i].x << " " << pts[i].y << std::endl;
    }
    return 0;
}